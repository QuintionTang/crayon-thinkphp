...
abstract class Driver {
    ...
    public function insert($data,$options=array(),$replace=false) {
        ...
        if ($db_type == "Pgsql" && $this->is_no_seq_tables($options['table'])){
            if ($result){
                $seq_name   = $this->parseTable($options['table']) . "_id_seq";
                
                $last_result = $this->last_insert_id($seq_name);
                if ($last_result){
                    $result = $last_result;
                }
            } else {    
    ...
    /**
     * [get_no_seq_tables 判断数据表是否在无序列数据表中]
     * @param  [type] $tablename [description]
     * @return [type]            [description]
     */
    private function is_no_seq_tables($tablename){
        $array_tables = C("DB_NOSEQ_TABLES");
        return !in_array($tablename,$array_tables);
    }
   ...
   /**
    ...
    public function last_insert_id($seq_name) {
        try{
            $sql  =   "select last_value from $seq_name";
            $result =   $this->query($sql);
            if ($result){
                $result = current($result);
                $last_insert_id = $result["last_value"];
